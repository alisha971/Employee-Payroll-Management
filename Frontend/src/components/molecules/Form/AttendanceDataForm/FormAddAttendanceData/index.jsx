...
const FormAddAttendanceData = () => {
    ...
    const filteredEmployeeData = EmployeeData.filter((employee) =>
        employee.emp_name.toLowerCase().includes(searchKeyword.toLowerCase())
    );
                        ...
                        <table className="w-full table-auto">
                            ...
                            <tbody>
                                {filteredEmployeeData.slice(startIndex, endIndex).map((data, index) => {
                                    const isNameAda = AttendanceData.some(
                                        (attendance) => attendance.emp_name === data.emp_name
                                    );
                    ...
                    <div className="flex justify-between items-center mt-4 flex-col md:flex-row md:justify-between">
                        <div className="flex items-center space-x-2">
                            <span className="text-gray-5 dark:text-gray-4 text-sm py-4">
                                Displaying {startIndex + 1}-{Math.min(endIndex, filteredEmployeeData.length)} from {filteredEmployeeData.length} Attendance Data employee
                            </span>
