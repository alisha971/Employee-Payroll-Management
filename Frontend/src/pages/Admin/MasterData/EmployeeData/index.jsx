...
const EmployeeData = () => {
    ...
    const filteredEmployeeData = EmployeeData.filter((employee) => {
        const { emp_name, status } = employee;
        const keyword = searchKeyword.toLowerCase();
        const statusKeyword = filterStatus.toLowerCase();
        return (
            emp_name.toLowerCase().includes(keyword) &&
                ...
                <div className="flex justify-between items-center mt-4 flex-col md:flex-row md:justify-between">
                    <div className="flex items-center space-x-2">
                        <span className="text-gray-5 dark:text-gray-4 text-sm py-4">
                            Displaying {startIndex + 1}-{Math.min(endIndex, filteredEmployeeData.length)} from {filteredEmployeeData.length} Employee Data
                        </span>
                    </div>
